...
public class DynamoDBServiceImpl implements DynamoDBService {
    ...
    public String updateUser(Map<String, String> pathParameters, String inputBody) {
        ...
        User existedUser = dynamoDBMapper.load(User.class, user.getEmail());

        if (existedUser == null) {
            return getJsonResponse("User not found");
        }

        // TODO validate social media resources if not null
        User userAttributesForUpdate = new Gson().fromJson(inputBody, User.class);
        userAttributesForUpdate.setCountry(INDEX_PARTITION_KEY_VALUE);

        updateUsersNotNullAttributes(existedUser, userAttributesForUpdate);

        dynamoDBMapper.save(existedUser);
